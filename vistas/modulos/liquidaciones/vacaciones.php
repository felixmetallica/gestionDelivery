<?php
	...
	$diasVacaciones = number_format((float)$_GET["dias"], 2, '.', '');

	$BaseVacacionesTemp = ($sueldoBasico/30)*$diasVacaciones;

	$BaseVacaciones = number_format((float)$BaseVacacionesTemp, 2, '.', '');
			...
			case '1':
				...
				switch ($value["Descripcion"]) {
					...
					case 'Sueldo Basico':
						...
						$valorConcepto = number_format((float)$BaseVacaciones, 2, '.', '');
						$totalHaberesRemunerativos = $BaseVacaciones;
						$unidades = number_format((float)$value["Unidades"]-$diasVacaciones, 2, '.', '');
					...
					case 'Antigüedad':
						...
						if ($antiguedad > 0) {
							...
							if ($value["Porcentaje"] =="N") {
								...
								$valorConceptoTemp = $unidades * $value["Unidades"];

								$valorConcepto = number_format((float)$valorConceptoTemp, 2, '.', '');
								$totalHaberesRemunerativos = $totalHaberesRemunerativos + $valorConcepto;

							} else {

								$porcentaje = ($BaseVacaciones * $value["Unidades"])/100;
					...
					case 'Vacaciones':
						...
						//VACACIONES
						$concepto = $value["Descripcion"];
						$valorConcepto = 0; //($sueldoBasico/25)*$diasVacaciones;
						$totalHaberesRemunerativos = $totalHaberesRemunerativos + $valorConcepto;
					...
					if ($concepto !="Antigüedad") {
						...
						if ($concepto == "Sueldo Basico") {
							...
							$clase = "sueldoBaseVacacion";
							$unidadSueldo = "unidadesSueldo";
							$valor = $BaseVacaciones;
						...
						if ($concepto == "Vacaciones") {
							...
							$valorVaca = 'valorVaca="'.$diasVacaciones.'"';
							$claseVaca = 'valorVaca';
							$claseVacaMonto = 'claseVacaMonto';
						...
						if ($concepto != "Aguinaldo") {
							...
							echo '
						 			<th scope="row" class="text-center">'.$value["Codigo"].'</th>
							  		<td><span id="nuevaDescripcionConcepto" class="nuevaDescripcionConcepto '.$claseVaca.'" idConcepto="'.$value["ConceptoID"].'" descripcionConcepto="'.$value["Descripcion"].'" tipoConcepto="'.$value["Tipo"].'" fijoConcepto="'.$value["Fijo"].'" '.$valorVaca.'>'.$value["Descripcion"].'</span></td>
							  		<td class="text-center"><span id="unidades" class="unidades '.$unidadSueldo.'" unidadesConcepto="'.$unidades.'">'.$unidades.'</span></td>
							  		<td class="text-center SueldoBasico" sueldo="'.number_format((float)$BaseVacaciones, 2, '.', '').'"><span id="sumarSueldo" class="sumarSueldo valorConcepto '.$claseVacaMonto.' '.$clase.'" valorConcepto="'.$valorConcepto.'">'.number_format((float)$valorConcepto, 2, '.', '').'</span></td>
							  		<td></span></td>
							  		<td></td>
							  </tr>';
							...
							$dtConceptos = array("ConceptoID" => $value["ConceptoID"],
							                 ...
							                 "Fijo" => $value["Fijo"],
							                 "Unidades" => $value["Unidades"],
							                 "Total" => number_format((float)$valor, 2, '.', ''));
						...
						if ($antiguedad > 0) {
							...
							echo '<tr>
						 			<th scope="row" class="text-center">'.$value["Codigo"].'</th>
							  		...
							  		<td class="text-center SueldoBasico Anti" sueldo="'.$BaseVacaciones.'" valorAntiguedad="'.number_format((float)$valorConcepto, 2, '.', '').'"><span id="sumarSueldo" class="sumarSueldo valorConcepto" valorConcepto="'.number_format((float)$valorConcepto, 2, '.', '').'">'.number_format((float)$valorConcepto, 2, '.', '').'</span></td>
							  		<td></span></td>
							  		<td></td>
							  </tr>';
	...
	//-------------------------------------------------------------------------------------------------------------------------------------------
			case '2':
				...
				if ($value["Porcentaje"] =="N") {
					...
					$unidades = $value["Unidades"];
					$valorConcepto = ($BaseVacaciones * $unidades)/100;
					$totalHaberesNoRemunerativos = $totalHaberesNoRemunerativos+$valorConcepto;
