<?php
	...
	$diasTrabajados = number_format((float)$_GET["cantidad"], 2, '.', '');

	$sueldoTemp = ($sueldoBasico/30)*$diasTrabajados;

	$sueldoParcial = number_format((float)$sueldoTemp, 2, '.', '');
			...
			case '1':
				...
				switch ($value["Descripcion"]) {
					...
					case 'Sueldo Basico':
						...
						$valorConcepto = number_format((float)$sueldoParcial, 2, '.', '');
						$totalHaberesRemunerativos = $sueldoParcial;
					...
					case 'Antigüedad':
						...
						if ($antiguedad > 0) {
							...
							if ($value["Porcentaje"] =="N") {
								...
								$porcentaje = ($sueldoParcial * $value["Unidades"])/100;
							...
							if ($concepto != "Aguinaldo") {
								...
								echo '
							 			<th scope="row" class="text-center">'.$value["Codigo"].'</th>
								  		...
								  		<td class="text-center SueldoBasico" sueldo="'.$sueldoParcial.'"><span id="sumarSueldo" class="sumarSueldo valorConcepto" valorConcepto="'.$valorConcepto.'">'.number_format((float)$valorConcepto, 2, '.', '').'</span></td>
								  		<td></td>
								  		<td></td>
								  </tr>';
						...
						if ($antiguedad > 0) {
							...
							echo '<tr>
						 			<th scope="row" class="text-center">'.$value["Codigo"].'</th>
							  		<td><span id="nuevaDescripcionConcepto" class="nuevaDescripcionConcepto" idConcepto="'.$value["ConceptoID"].'" descripcionConcepto="'.$value["Descripcion"].'" tipoConcepto="'.$value["Tipo"].'" fijoConcepto="'.$value["Fijo"].'">'.$value["Descripcion"].'</span></td>
							  		<td class="text-center"><span id="unidades" class="unidades" unidadesConcepto="'.$unidades.'">'.$unidades.'</span></td>
							  		<td class="text-center SueldoBasico Anti" sueldo="'.$sueldoParcial.'" valorAntiguedad="'.number_format((float)$valorConcepto, 2, '.', '').'"><span id="sumarSueldo" class="sumarSueldo valorConcepto" valorConcepto="'.number_format((float)$valorConcepto, 2, '.', '').'">'.number_format((float)$valorConcepto, 2, '.', '').'</span></td>
							  		<td></span></td>
							  		<td></td>
							  </tr>';
	...
	//-------------------------------------------------------------------------------------------------------------------------------------------
			case '2':
				...
				if ($value["Porcentaje"] =="N") {
					...
					$unidades = $value["Unidades"];
					$valorConcepto = ($sueldoParcial * $unidades)/100;
					$totalHaberesNoRemunerativos = $totalHaberesNoRemunerativos+$valorConcepto;
